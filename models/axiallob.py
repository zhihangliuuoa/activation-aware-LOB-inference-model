...
class GatedAxialAttention(nn.Module):
    """Gated Axial Attention from the project"""
    ...
    def forward(self, x):
        ...
        stacked_similarity = self.bn_similarity(stacked_similarity).view(
            N * W, 3, self.heads, H, H).sum(dim=1)
...
class AxialLOB(nn.Module):
    """
    Axial-LOB model from the project
    Input shape: (batch, 1, 40, 40)
    """
    def __init__(self, W=40, H=40, c_in=32, c_out=32, c_final=4, n_heads=4,
                 pool_kernel=(1, 4), pool_stride=(1, 4), num_classes=3):
        super().__init__()
        ...
        self.linear = nn.Linear(c_final * H * (W // pool_stride[1]), num_classes)
    ...
    @staticmethod
    def get_input_shape():
        """返回模型所需的输入形状 (channels, height, width)"""
        return (1, 40, 40)
